...
{
    size_t operator()(const labelset<T> &k) const
    {
        size_t tmp = 0;
        for(int i=0;i<k.data;i++)
        {
            if(i&1){
                tmp<<=1;
                tmp^=(k.dt[i]>>1);
...
unordered_map<int, NFA_VERTEX*> NFA_GRAPH;
unordered_map<labelset<int>, DFA_VERTEX*, LabelsetHasher<int> > DFA_GRAPH;
...
//        printf("%s\nlength = %d\n", regexp, len);
		...
		unordered_map<labelset<int>, DFA_VERTEX*, LabelsetHasher<int> >::iterator it_dfa;
		unordered_map<int, NFA_VERTEX*>::iterator it_nfa;
